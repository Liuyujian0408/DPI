...
def val(model, dataloader, device):
    ...
    pred_list = []
    label_list = []
    for bg, label, mol_point_xyzs, clu_indices_mol, pocket_point_xyzs, clu_indices_pock, pocket_mol_idx, keys in dataloader:
        ...
        with torch.no_grad():
            pred_lp, pred_pl, _ = model(bg, xyz, indices, device)
...
if __name__ == '__main__':
    ...
    valid_dir = data_root
    valid_df = pd.read_csv(os.path.join(data_root, 'toy_examples.csv'))


    val_paths = point_constrctor(valid_df, valid_dir)

    device = torch.device('cuda:0')
    model = ECloudBind(node_feat_size=35, edge_feat_size=17,
                         hidden_feat_size=256, layer_num=1, droup_out=0.1).to(device)

    load_model_dict(model, 'weights/best_model_ECloudbind.pt')

    valid_set = GraphDataset(valid_dir, valid_df, graph_type='Graph_E_cloudBind',
                             point_path=val_paths, create=False)


    valid_loader = DataLoader(valid_set, batch_size=48, shuffle=False, collate_fn=collate_fn, num_workers=8)


    valid_rmse, valid_pr = val(model, valid_loader, device)


    msg = "valid_rmse: %.4f, valid_pr: %.4f " % (valid_rmse, valid_pr)
    print(msg)
