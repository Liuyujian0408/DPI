...
def point_constrctor(train_df):
    point_dir = 'data/DPI_data_cluster30'
    protein_keys = os.listdir(
        os.path.join(point_dir, 'DPI_pocket_cloud_fixed(cluster)_num_multivariate_normal/TrainVal'))
    mol_point_paths_train = {}
    pocket_point_paths_train = {}
    pocket_xyz_train = {}

    mol_point_paths_val = {}
    pocket_point_paths_val = {}
    pocket_xyz_val = {}
    for i, row in train_df.iterrows():
        cid, pKa = row['pdbid'], float(row['-logKd/Ki'])
        if f'{cid}.pkl' in protein_keys:
            mol_point_paths_train[cid] = os.path.join(point_dir,
                                                      'point_cloud_with_clus',
                                                      'TrainVal',
                                                      cid)
            pocket_point_paths_train[cid] = os.path.join(point_dir,
                                                         'DPI_pocket_cloud_fixed(cluster)_num_multivariate_normal',
                                                         'TrainVal',
                                                         f"{cid}.pkl")
            pocket_xyz_train[cid] = os.path.join(point_dir,
                                                 'DPI_complex_pkl',
                                                 'TrainVal',
                                                 cid,
                                                 f"{cid}.pkl")
    train_paths = [mol_point_paths_train, pocket_point_paths_train, pocket_xyz_train]
    val_paths = [mol_point_paths_val, pocket_point_paths_val, pocket_xyz_val]
...
def mols2graphs(complex_path, label, save_path, dis_threshold=5.0):
    try:
        ...
        (edge_index_l2p, edge_attr_l2p), (edge_index_p2l, edge_attr_p2l) = inter_graph(ligand, pocket,
...
if __name__ == '__main__':
    ...
    toy_set = GraphDataset(toy_dir, toy_df, dis_threshold=5., create=True)
